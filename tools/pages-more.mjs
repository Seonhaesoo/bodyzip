...
<p class="note">100일은 태어난 날을 1일로 세어 100일째(생후 99일 뒤)이고, 돌은 달력상 같은 날짜입니다. 카드 이미지는 브라우저 안에서 그려지며 서버로 전송되지 않습니다.</p>` }));
  ...
  for (const sex of ['m', 'f']) for (const m of PCT_MONTHS) {
    ...
    const desc = `${sexName(sex)} ${m}개월 평균(50백분위)은 몸무게 ${med(W)}kg, 키 ${med(H)}cm, 머리둘레 ${med(HC)}cm이고 정상 범위(3~97백분위)는 몸무게 ${lo(W)}~${hi(W)}kg, 키 ${lo(H)}~${hi(H)}cm입니다. WHO 표준·질병관리청 성장도표 기준, 내 아기 수치를 넣으면 백분위가 바로.`;
...
${lead(`${sexName(sex)} ${m}개월의 한가운데(50백분위)는 몸무게 <b>${med(W)}kg</b>, 키 <b>${med(H)}cm</b>입니다. 3백분위(${lo(W)}kg·${lo(H)}cm)부터 97백분위(${hi(W)}kg·${hi(H)}cm)까지는 모두 정상 범위이고, 백분위는 또래 100명 중 몇 번째로 큰지를 뜻합니다. 내 아기 수치를 넣으면 정확한 백분위가 나옵니다.`)}
${pctForm(sex, m)}
...
<p><b>재는 법.</b> 몸무게는 기저귀를 벗기고, 키는 24개월 전에는 눕혀서(신장), 이후는 세워서(신장) 잽니다. 머리둘레는 눈썹 위~뒤통수 가장 튀어나온 곳.</p>
</div>`)}
...
${pctForm('m', 6)}
${lead('백분위는 같은 성별·개월의 아기 100명 중 몇 번째로 큰지입니다. 50이 한가운데, 3~97 사이면 정상 범위입니다. 생년월일을 넣으면 개월이 자동으로 계산되고, 개월을 누르면 남아·여아 백분위표가 나옵니다.')}
...
${section('이어서', null, list([{ href: '/baby/month/', title: '아기 개월별 발달', sub: '할 수 있는 것 · 수유 · 수면' }, { href: '/child-height/', title: '아이 예상 키', sub: '부모 키로' }, { href: '/baby/card/', title: '아기 100일·돌 카드', sub: '공유용 이미지' }]))}` }));
  ...
  for (const k of CAFFEINE_PAGES) for (const n of CAFFEINE_COUNTS) {
    ...
    const title = `${c.label.split(' (')[0]} ${n}잔 카페인 — ${mg}mg, 성인 하루 기준 400mg의 ${Math.round(mg / 400 * 100)}% (임신 300mg)`;
    const desc = `${c.label} ${n}잔의 카페인은 약 ${mg}mg입니다. 성인 하루 권고 400mg의 ${Math.round(mg / 400 * 100)}%, 임신·수유 중 300mg의 ${Math.round(mg / 300 * 100)}%. 오후 2시에 마셨다면 밤 11시에 ${left9}mg이 남습니다(반감기 5시간).`;
    const body = `
${crumb([['/caffeine/', '카페인'], [null, `${c.label.split(' (')[0]} ${n}잔`]])}
<h1 class="title">${c.label.split(' (')[0]} ${n}잔</h1>
<p class="meta">${c.label} · 잔당 약 ${c.mg}mg · 반감기 약 5시간</p>
${tiles([{ label: '카페인', value: `${mg}mg` }, { label: '성인 400mg 대비', value: `${Math.round(mg / 400 * 100)}%` }, { label: '임신 300mg 대비', value: `${Math.round(mg / 300 * 100)}%` }])}
${lead(`${c.label.split(' (')[0]} ${n}잔에는 카페인이 약 <b>${mg}mg</b> 들어 있습니다. 식약처 성인 하루 최대 섭취 권고량 400mg의 ${Math.round(mg / 400 * 100)}%${mg > 400 ? '로 기준을 넘습니다' : '입니다'}. 카페인은 5시간마다 절반씩 줄어, 오후 2시에 마셨다면 밤 11시에도 ${left9}mg이 남아 있습니다.`)}
${cafForm(k, n)}
${section('시간이 지나면', `${mg}mg · 반감기 5시간`, table(['지난 시간', '남은 카페인'], [0, 3, 5, 8, 10, 12, 15].map((h) => ({ cells: [h === 0 ? '마신 직후' : `${h}시간 뒤`, `${X.caffeineLeft(mg, h)}mg`] }))))}
${ad()}
${section('다른 양', null, wrapChips(CAFFEINE_COUNTS.map((x) => ({ label: `${x}잔`, href: cafUrl(k, x), on: x === n }))))}
${section('다른 음료', `${n}잔`, wrapChips(CAFFEINE_PAGES.filter((x) => x !== k).map((x) => ({ label: CF[x].label.split(' (')[0], href: cafUrl(x, n) }))))}
${section('알아두면 좋은 것', null, `<div class="doc">
<p><b>기준.</b> 식약처 하루 최대 섭취 권고량은 성인 400mg, 임산부 300mg, 어린이·청소년은 몸무게 1kg당 2.5mg(50kg이면 125mg)입니다.</p>
<p><b>잠에 영향을 주지 않으려면 자기 6시간 전까지.</b> 6시간 뒤에도 절반 가까이 남아 있어, 밤 11시에 자려면 오후 5시 이후 카페인은 피하는 것이 좋습니다. 카페인 대사가 느린 사람은 더 이릅니다.</p>
...
${section('이어서', null, list([{ href: '/sleep/', title: '몇 시에 자야 할까', sub: '수면 주기' }, { href: '/food/', title: '음식 칼로리 사전', sub: '커피 칼로리' }, { href: '/pregnancy/week/', title: '임신 주차별 안내', sub: '임신 중 카페인 300mg' }]))}` }));
  ...
  for (const d of QUIT_DAYS) {
    ...
    const title = `금연 ${d}일 — 하루 한 갑이면 ${num(q.cigs)}개비, ${num(q.money)}원 아낌 · 몸의 변화: ${st.label}`;
...
<p><b>보건소 금연클리닉은 무료.</b> 니코틴 패치·껌과 상담을 12주 지원하고, 병원 금연치료도 건강보험이 됩니다(1588-5333 금연상담전화).</p>
<p><b>한 대 피웠다고 실패가 아닙니다.</b> 대부분 5~7번 시도 끝에 성공합니다. 날짜를 다시 넣고 이어 가세요.</p>
</div>`)}
...
${NOTE('되찾은 시간은 개비당 약 20분(University College London, 2024)이며 통계적 추정입니다. 몸의 변화 시점은 미국 CDC·보건복지부 금연길라잡이의 일반 안내입니다.')}`;
...
${section('날짜별', '하루 한 갑 · 4,500원', table(['금연', '안 피운 담배', '모은 돈', '몸의 변화'], QUIT_DAYS.map((d) => { const q = X.quitStats(d, 20, 4500); return { cells: [`<a href="${quitUrl(d)}">${d}일</a>`, `${num(q.cigs)}개비`, `${num(q.money)}원`, X.quitStage(d).label] }; })))}
${ad()}
