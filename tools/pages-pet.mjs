...
export function buildPet(ctx) {
  ...
  for (const kind of ['dog', 'cat']) {
    ...
    for (const y of YEARS) {
      ...
      const desc = kind === 'dog' ? `강아지 ${y}살은 사람 나이로 소형견 ${h}세, 중형견 ${hm}세, 대형견 ${hl}세에 해당합니다(1살 15세, 2살 24세, 이후 크기별 4·5·6세씩). ${P.petStage('dog', y, 'small')}.` : `고양이 ${y}살은 사람 나이로 약 ${h}세입니다(1살 15세, 2살 24세, 이후 해마다 4세씩). ${P.petStage('cat', y)}.`;
      const body = `
...
${lead(kind === 'dog' ? `강아지 ${y}살은 사람으로 치면 소형견 <b>${h}세</b>, 중형견 ${hm}세, 대형견 ${hl}세 정도입니다. 큰 개일수록 빨리 늙어 같은 나이라도 사람 나이가 높습니다. ${P.petStage('dog', y, 'small')}.` : `고양이 ${y}살은 사람으로 치면 약 <b>${h}세</b>입니다. 첫 두 해에 24세까지 자라고 그 뒤로는 해마다 4세씩 더합니다. ${P.petStage('cat', y)}.`)}
${ageForm(kind, y)}
...
${kind === 'dog' ? '<p><b>큰 개가 더 빨리 늙습니다.</b> 대형견은 7세부터 노령견 관리(연 2회 검진, 관절·심장)를 시작하고, 소형견은 10세 무렵부터입니다.</p><p><b>2019년 UCSD 연구</b>는 DNA 메틸화로 사람 나이 = 16 × ln(개 나이) + 31 이라는 공식을 냈습니다(래브라도 기준). 1살이 31세, 4살이 53세로 어릴 때 더 가파릅니다. 위 표와 함께 참고로 보세요.</p>' : '<p><b>실내 고양이는 15~20년</b>을 삽니다. 11세부터 노령묘로 보고 신장·갑상선·치아 검진을 6개월마다 권합니다.</p>'}
</div>`)}
...
${NOTE('미국수의사회(AVMA)·AKC 환산표 기준. 품종·건강 상태에 따라 다릅니다.')}`;
...
${section('이어서', null, list([{ href: `/pet/${kind}-food/`, title: `${kindName(kind)} 사료량`, sub: '몸무게별' }, { href: `/pet/${kind}-vaccine/`, title: `${kindName(kind)} 예방접종 일정`, sub: '캘린더 파일' }]))}` }));
  ...
  for (const kind of ['dog', 'cat']) {
    ...
    for (const kg of KGS) {
      ...
      const desc = `몸무게 ${kg}kg ${kindName(kind)}의 하루 사료량은 중성화 ${kind === 'cat' ? '성묘' : '성견'} 기준 약 ${r.grams}g(${num(r.der)}kcal, 사료 100g당 ${P.KCAL_PER_100G}kcal)입니다. 기초 열량 RER ${num(r.rer)}kcal에 상태 계수를 곱해 계산합니다. 강아지·노령·감량 중 표.`;
      const body = `
...
${lead(`몸무게 ${kg}kg ${kindName(kind)}가 가만히 있어도 쓰는 기초 열량(RER)은 ${num(r.rer)}kcal이고, 중성화한 성체는 여기에 ${r.factor.f}을 곱한 <b>${num(r.der)}kcal</b>, 사료로는 하루 약 <b>${r.grams}g</b>(두 끼면 ${r.perMeal2}g씩)입니다. 사료 봉지의 100g당 열량이 다르면 아래 계산기에 넣으세요.`)}
${foodForm(kind, kg)}
...
<p><b>간식은 하루 열량의 10% 이내.</b> 간식을 주면 그만큼 사료를 빼세요. 사료 열량은 봉지의 "대사에너지(ME) kcal/kg"을 100으로 나누면 100g당 값입니다.</p>
</div>`)}
${section('이어서', null, list([{ href: `/pet/${kind}-age/`, title: `${kindName(kind)} 나이 환산`, sub: '사람 나이로' }, { href: `/pet/${kind}-vaccine/`, title: `${kindName(kind)} 예방접종 일정`, sub: '캘린더 파일' }, { href: '/food/', title: '사람 음식 칼로리', sub: '치킨 한 조각은 강아지에게 너무 많습니다' }]))}
${NOTE('RER·DER 계산은 WSAVA 영양 가이드라인의 일반 공식이며 계수는 개체·품종·질환에 따라 수의사가 조정합니다.')}`;
      write(url, shell({ url, title, desc, body, nav: 'pet', scripts: ['/js/engine.js', '/js/live.js'] }));
    }
    write(`/pet/${kind}-food/`, shell({ url: `/pet/${kind}-food/`, title: `${kindName(kind)} 사료량 계산기 — 몸무게·상태별 하루 급여량 (g·kcal)`, desc: `${kindName(kind)} 몸무게와 상태(중성화·강아지·노령·감량)를 넣으면 하루 열량과 사료 그램이 나옵니다. RER 70×kg^0.75 공식, ${kind === 'dog' ? '1~50' : '1~12'}kg 표.`, nav: 'pet', scripts: ['/js/engine.js', '/js/live.js'], body: `
...
${section('이어서', null, list([{ href: `/pet/${kind}-age/`, title: `${kindName(kind)} 나이 환산`, sub: '' }, { href: `/pet/${kind}-vaccine/`, title: `${kindName(kind)} 예방접종 일정`, sub: '' }]))}` }));
  ...
  for (const kind of ['dog', 'cat']) {
    for (const birth of dates) {
      ...
      const title = `${D.fmt(birth)}생 ${kindName(kind)} 예방접종 일정 — 오늘 ${weeks}주, 다음 접종 ${nextDose ? `${nextDose.name.split(' (')[0]} ${D.fmtShort(nextDose.dt)}` : '연간 추가 접종'}`;
...
${vacForm(kind, birth)}
${lead(`${D.fmt(birth)}에 태어난 ${kindName(kind)}는 오늘 생후 <b>${weeks}주</b>입니다. ${kind === 'dog' ? '종합백신은 6주부터 2주 간격으로 5회, 광견병은 16주에 맞고' : '종합백신은 8주부터 3주 간격으로 3회, 광견병은 16주에 맞고'} 심장사상충 예방약은 ${D.fmtShort(P.heartwormStart(birth))}(8주)부터 매달 먹입니다. 아래 표를 캘린더 파일로 받으면 하루 전 알림이 옵니다.`)}
...
<p><b>심장사상충·외부기생충 예방약은 평생 매달.</b> 모기가 없는 겨울에도 거르지 않는 것이 국내 수의사들의 권고입니다. 중성화는 ${kind === 'dog' ? '5~6개월(대형견은 성장 뒤)' : '5~6개월'} 무렵 상담합니다.</p>
</div>`)}
...
${lead('강아지 5살은 사람 36세, 5kg 강아지의 하루 사료는 약 160g. 생일을 넣으면 예방접종 날짜가 나오고 캘린더에 한 번에 넣을 수 있습니다.')}
