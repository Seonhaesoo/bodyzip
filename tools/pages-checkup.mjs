...
export const ALT = range(5, 200, 5);
export const URIC = range(2, 12, 0.2);

const NOTE_MED = '검진 수치는 한 번의 결과만으로 진단하지 않습니다. 재검·문진·다른 검사를 함께 보고 의사가 판단하며, 이 페이지는 공개된 학회 기준을 정리한 참고 자료입니다.';
...
${form}
${lead('검진 결과지에는 숫자와 참고치만 적혀 있어 "그래서 어떤 상태인가"를 알기 어렵습니다. 수치를 넣으면 학회 기준으로 어느 구간인지, 무엇을 하면 되는지 한 줄씩 붙여 드립니다. 항목별로 자세히 보려면 아래에서 수치를 누르세요.')}
...
${NOTE('')}` }));
  ...
  for (const s of SYS) for (const d of DIA) {
    ...
    const title = `혈압 ${s}/${d} — ${r.label}${r.high ? ' (진료 필요 구간)' : ''}, 어떻게 봐야 할까`;
...
<form class="quick live" data-live="bp" style="margin-top:14px"><div class="live-head"><b>다시 재 보기</b><span>두 번 이상 재고 평균을 봅니다</span></div><div class="ye-grid"><label class="ye-f"><span>수축기</span><input data-k="sys" type="text" inputmode="numeric" value="${s}"></label><label class="ye-f"><span>이완기</span><input data-k="dia" type="text" inputmode="numeric" value="${d}"></label></div><div class="tiles"><div class="tile"><small>판정</small><span class="num" data-out="label"></span></div><div class="tile"><small>맥압</small><span class="num" data-out="pulse"></span></div><div class="tile"><small>한마디</small><span class="num" data-out="note"></span></div></div><div class="live-foot"><a data-out="link" href="/bp/120-80/">이 수치 페이지 →</a></div></form>
${section('혈압 구간표', '대한고혈압학회 2022', table(['구간', '수축기', '', '이완기'], [
  ...
  { cells: ['고혈압 2기', '160 이상', '또는', '100 이상'], cls: r.key === 'stage2' ? 'on' : '' },
]))}
${ad()}
${section('수축기가 다르면', `이완기 ${d} 고정`, chips(near(SYS, s).map((x) => ({ label: `${x}/${d}`, href: bpUrl(x, d), on: x === s }))))}
${section('이완기가 다르면', `수축기 ${s} 고정`, chips(near(DIA, d).map((x) => ({ label: `${s}/${x}`, href: bpUrl(s, x), on: x === d }))))}
${section('알아두면 좋은 것', null, `<div class="doc">
<p><b>재는 법이 결과를 바꿉니다.</b> 5분 앉아 쉰 뒤, 팔을 심장 높이에 두고, 커피·담배·운동 30분 뒤에 잽니다. 1~2분 간격으로 두 번 재서 평균을 씁니다.</p>
<p><b>병원에서만 높다면</b> 백의 고혈압일 수 있습니다. 집에서 아침·저녁으로 일주일 재 보세요. 가정혈압은 135/85 이상이면 고혈압으로 봅니다.</p>
<p><b>소금과 체중이 가장 크게 움직입니다.</b> 하루 소금 5g 줄이면 수축기가 약 5mmHg, 체중 1kg 줄이면 약 1mmHg 내려갑니다.</p>
</div>`)}
${section('이어서', null, list([{ href: '/checkup/', title: '검진 결과 전체 해석', sub: '혈당·콜레스테롤까지 한 번에' }, { href: '/bmi/', title: 'BMI · 정상 체중', sub: '체중이 혈압을 좌우합니다' }, { href: '/steps/', title: '걸음 수 칼로리', sub: '유산소가 가장 잘 듣습니다' }]))}
...
<form class="quick live" data-live="bp" style="margin-top:14px"><div class="live-head"><b>내 혈압</b><span>바꾸면 바로</span></div><div class="ye-grid"><label class="ye-f"><span>수축기</span><input data-k="sys" type="text" inputmode="numeric" value="120"></label><label class="ye-f"><span>이완기</span><input data-k="dia" type="text" inputmode="numeric" value="80"></label></div><div class="tiles"><div class="tile"><small>판정</small><span class="num" data-out="label"></span></div><div class="tile"><small>맥압</small><span class="num" data-out="pulse"></span></div><div class="tile"><small>한마디</small><span class="num" data-out="note"></span></div></div><div class="live-foot"><a data-out="link" href="/bp/120-80/">이 수치 자세히 →</a></div></form>
${lead('혈압은 수축기(위)와 이완기(아래) 둘 다 봅니다. 둘 중 하나라도 높은 구간에 들면 그 구간으로 판정합니다. 아래 표에서 내 수치를 누르면 자세한 설명이 나옵니다.')}
${section('많이 찾는 수치', null, chips([[110, 70], [120, 80], [130, 80], [130, 85], [140, 90], [145, 95], [150, 90], [160, 100]].map(([a, b]) => ({ label: `${a}/${b}`, href: bpUrl(a, b) }))))}
${section('수축기별 (이완기 80 기준)', null, chips(SYS.map((x) => ({ label: `${x}`, href: bpUrl(x, 80) }))))}
${ad()}
${section('이어서', null, list([{ href: '/checkup/', title: '검진 결과 전체 해석', sub: '' }, { href: '/glucose/100/', title: '공복혈당', sub: '' }]))}` }));

  /* ---------- 값 하나짜리 항목들 ---------- */
  const single = [
    { key: 'glucose', dir: '/glucose/', name: '공복혈당', unit: 'mg/dL', arr: GLU, fn: C.glucose, live: 'glucose',
      meta: '대한당뇨병학회 2023 · 8시간 공복 후 정맥혈',
      table: [['정상', '100 미만'], ['공복혈당장애 (당뇨 전단계)', '100~125'], ['당뇨병', '126 이상 (두 번 확인)']],
...
<p><b>검사 전날 저녁이 중요합니다.</b> 늦은 야식과 음주는 다음 날 공복혈당을 10~20 올립니다.</p>`,
    ...
    { key: 'cholesterol', dir: '/cholesterol/', name: '총콜레스테롤', unit: 'mg/dL', arr: TC, fn: C.totalChol, live: 'chol',
      ...
      table: [['적정', '200 미만'], ['경계', '200~239'], ['높음', '240 이상']],
...
<p><b>음식보다 체중과 운동이 큽니다.</b> 콜레스테롤의 70~80%는 몸에서 만들어집니다. 포화지방(삼겹살·버터·튀김)을 줄이는 것이 달걀을 피하는 것보다 효과적입니다.</p>`,
    ...
    { key: 'ldl', dir: '/ldl/', name: 'LDL 콜레스테롤', unit: 'mg/dL', arr: LDL, fn: C.ldl, live: 'ldl',
      ...
      table: [['적정', '100 미만'], ['정상', '100~129'], ['경계', '130~159'], ['높음', '160~189'], ['매우 높음', '190 이상']],
      tips: `<p><b>목표치는 사람마다 다릅니다.</b> 당뇨·심근경색 병력이 있으면 70 미만, 위험 요인이 많으면 100 미만을 목표로 잡습니다. 건강한 사람은 130 미만이면 대체로 괜찮습니다.</p>
<p><b>포화지방과 트랜스지방을 줄이면 내려갑니다.</b> 튀김·가공육·버터·과자를 줄이고 통곡물·견과류·등푸른생선을 늘리세요.</p>`,
    ...
    { key: 'hdl', dir: '/hdl/', name: 'HDL 콜레스테롤', unit: 'mg/dL', arr: HDL, fn: C.hdl, live: 'hdl',
      ...
      table: [['낮음 (위험)', '40 미만'], ['보통', '40~59'], ['높음 (좋음)', '60 이상']],
      tips: `<p><b>HDL은 높을수록 좋습니다.</b> 혈관에 쌓인 콜레스테롤을 간으로 실어 나르는 역할을 합니다. 40 미만이면 다른 수치가 정상이어도 위험이 올라갑니다.</p>
<p><b>가장 잘 듣는 것은 유산소 운동과 금연입니다.</b> 주 3회 30분 이상 빠르게 걷기, 금연 후 몇 주 안에 오르기 시작합니다. 과도한 정제 탄수화물은 HDL을 낮춥니다.</p>`,
    ...
    { key: 'tg', dir: '/triglyceride/', name: '중성지방', unit: 'mg/dL', arr: TG, fn: C.triglyceride, live: 'tg',
      ...
      table: [['적정', '150 미만'], ['경계', '150~199'], ['높음', '200~499'], ['매우 높음', '500 이상']],
...
<p><b>500 이상은 급성 췌장염 위험</b>이 있어 바로 진료가 필요합니다.</p>`,
    ...
    { key: 'uric', dir: '/uric/', name: '요산', unit: 'mg/dL', arr: URIC, fn: (v) => C.uric(v, 'm'), live: 'uric',
      ...
      table: [['정상 (남)', '3.4~7.0'], ['정상 (여)', '2.4~6.0'], ['고요산혈증', '남 7.0 · 여 6.0 초과'], ['통풍 위험 높음', '9.0 초과']],
...
<p><b>급격한 체중 감량도 요산을 올립니다.</b> 단식보다 천천히 빼는 편이 안전합니다.</p>`,
  ...
  for (const it of single) {
    for (const v of it.arr) {
      ...
      const title = `${it.name} ${v} — ${r.label}, 무엇을 하면 될까 (정상 수치와 비교)`;
      const desc = `${it.name} ${v}${it.unit}는 ${r.label} 구간입니다. ${r.note} 정상 기준과 구간표, 낮추는 방법.`;
...
${lead(`${it.name} <b>${v}${it.unit}</b>는 ${r.label} 구간입니다. ${r.note}`)}
<form class="quick live" data-live="${it.live}" style="margin-top:14px"><div class="live-head"><b>내 수치</b><span>바꾸면 바로</span></div><div class="ye-grid"><label class="ye-f"><span>${it.name} (${it.unit})</span><input data-k="v" type="text" inputmode="decimal" value="${v}"></label>${it.key === 'uric' ? '<label class="ye-f"><span>성별</span><select data-k="sex"><option value="m">남</option><option value="f">여</option></select></label>' : ''}</div><div class="tiles"><div class="tile"><small>판정</small><span class="num" data-out="label"></span></div><div class="tile"><small>한마디</small><span class="num" data-out="note"></span></div></div><div class="live-foot"><a data-out="link" href="${it.dir}">이 수치 페이지 →</a></div></form>
${section('구간표', null, table(['구간', it.unit], it.table.map(([a, b]) => ({ cells: [a, b], cls: r.label.startsWith(a.split(' ')[0]) ? 'on' : '' }))))}
${ad()}
...
<p class="meta">${it.meta}</p>
<form class="quick live" data-live="${it.live}" style="margin-top:14px"><div class="live-head"><b>내 수치</b><span>바꾸면 바로</span></div><div class="ye-grid"><label class="ye-f"><span>${it.name} (${it.unit})</span><input data-k="v" type="text" inputmode="decimal" value="${it.arr[Math.floor(it.arr.length / 2)]}"></label>${it.key === 'uric' ? '<label class="ye-f"><span>성별</span><select data-k="sex"><option value="m">남</option><option value="f">여</option></select></label>' : ''}</div><div class="tiles"><div class="tile"><small>판정</small><span class="num" data-out="label"></span></div><div class="tile"><small>한마디</small><span class="num" data-out="note"></span></div></div><div class="live-foot"><a data-out="link" href="${it.dir}">이 수치 자세히 →</a></div></form>
...
${NOTE('')}` }));
  ...
  for (const a of ALT) {
    const r = C.liver(a, a), url = liverUrl(a);
...
${section('구간표', 'AST·ALT 중 높은 값 기준', table(['구간', 'IU/L'], [
  ...
  { cells: ['고도 상승', '200 초과'], cls: r.key === 'severe' ? 'on' : '' },
]))}
${ad()}
${section('가까운 수치', null, chips(near(ALT, a).map((x) => ({ label: `${x}`, href: liverUrl(x), on: x === a }))))}
${section('알아두면 좋은 것', null, `<div class="doc">
<p><b>가장 흔한 원인은 지방간입니다.</b> 우리나라 성인의 간수치 상승 중 대부분이 비알코올성 지방간이고, 체중을 7~10% 줄이면 수치가 정상으로 돌아오는 경우가 많습니다.</p>
